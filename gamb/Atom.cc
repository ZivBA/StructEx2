...
string Atom::parseAtomName(const char* const atomName_) {
  string atomname;
  switch (strlen(atomName_)) {
  case 1:
    atomname = " " + string(atomName_) + "  ";
    break;
  case 2:
    atomname = " " + string(atomName_) + " ";
    break;
  case 3:
    ...
    break;
  default:
    atomname = string(atomName_).substr(0,4);
  }
...
Atom::Atom()
  : Vector3(), atomEntryType(ATOM), chId(' '), altLoc(' '), atomId(0),
    ...
    occupancy(defaultOccupancy), tempFactor(defaultTempFactor),sseInfo(pair<char,short>(0,0))
{}
  ...
  occupancy(defaultOccupancy), tempFactor(defaultTempFactor),sseInfo(pair<char,short>(0,0))
{
  ...
  resName = string(PDB::residueLongName(resTp)).substr(0,3);
}
...
Atom::Atom(const Vector3& p, const AtomEntryType atp, const char ch,
  ...
  Vector3(p), atomEntryType(atp), chId(ch), altLoc(' '), atomId(aid),
  residueId(resid), residueSeqICode(resICode), resType(resTp), resName("   "),
  occupancy(occup), tempFactor(tmp), sseInfo(pair<char,short>(0,0))
{
  atomName = parseAtomName(atomName_);
  resName = string(res).substr(0,3);
}
...
ostream& operator<<(ostream& s, const Atom& at)
{
