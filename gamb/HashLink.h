...
struct GeomHashParams {
  GeomHashParams(int dim, vector<float> cube_sizes, float resize_coefficient)
    : dimension(dim), cubeSizes(cube_sizes), resizeCoefficient(resize_coefficient)
  {
    if (cubeSizes.size() != (unsigned int) dimension) {
      cerr << "Invalid Parameter: the size of the cube_sizes vector has to be equal to the dimension of the hash" << endl;
      exit(1);
    }
  }
...
{
public:
  typedef vector<const Bucket* > BucketsPointerList;

private:
  HashLink(const HashLink<KeyT, DataT, Bucket>&){
  	cerr<<"HashLinkCC"<<endl;exit(0);}
protected:
...
template<class KeyT, class DataT, class Bucket>
void HashLink<KeyT, DataT, Bucket>::get(const KeyT &key, const vector<float>& radii,
                                        const GeomHashParams& params, int axis,
                                        HashResult<DataT, Bucket>& result) const
{
  if (next) {
