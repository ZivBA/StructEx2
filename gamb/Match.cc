...
ostream& operator<<(ostream& s, const ParticleMatch& pm)
{
...
ostream& operator<<(ostream& s, const Match& m)
{
  ...
  s << "#model  #scene  #score   #priority" << endl;
  for(vector<ParticleMatch>::const_iterator it=m.pairs.begin() ;
      it != m.pairs.end(); ++it)
    s << *it << endl;
  s << endl;
  s << "#The transformation is:"  << endl;
  s << m.trans << endl;
  s << "#score: " << m.sc  << "#Rms: " << m.rms << endl;
  return s;
