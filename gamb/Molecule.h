...
CHANGES LOG
  <UL>
  <LI>
  6.04.08, Dina: Adding BackboneSelector
  </LI>
  <LI>
  20.06.06, Dina: Adding CaSelector
  </LI>
  <LI>
  10.05.05, Nelly Bluvshtein: Adding WaterHydrogenUnSelector
  </LI>
  <LI>
  7.11.04, Alex Shulman: Changing the methods that read PDB to templates
  of general streams (e.g zipped or regular files). Removing the
  repositioning of file pointer to the beginning of the file with
  seekg. If the user does not want to manage the file
  pointer by itself, functions that receive the name of the file must be
  used.

  </LI>
  <LI>
  21.09.04, Oranit Dror: Adding a new 'readAllPDBfile' method.
  </LI>
  <LI>
  25.08.04, Oranit Dror: Adding a new 'readPDBfile' method.
  </LI>
  <LI>
  3.03.04 readPDB functions: line, 85 was changed to MAX_LINE_LENGTH = 1000
  </LI>
  <LI>
  13.01.03 PDBstream.getline(line,85); //PDBstream.get(line, 85, '\n');
  </LI>
  <LI>
  New readPDB functions. One of them reads only HETATM (it was not possible
  with previous readPDBFile function). The second function reads all atoms:
  both ATOM and HETATM. The selectors can be used with them like with readPDBFile.
  8.01.03 Alex and Dina
  </LI>
  <LI>
  The istream pointer in readPDBfile is set to the
  beggining of the file in the beggining of this method.
  It enables multiple readings of the same stream.
  11.11.2002 Inbal Halperin.
  </LI>
  <LI>
  Added a Selector. This selector is different from the PDB::Selector.
  The PDB::Selector can be operated only when a molecule is read.
  This selector can either select particles from a given Molecule object
  which was already created, thereby changing the molecule or
  can select particles into a new molecule without changing the
  operating object molecule. 7.11.2002 Inbal Halperin.
  </LI>
  <LI>
  getline is changes to StringUtils::getLine
  </LI>
  </UL>
...
public:
    ...
    int readPDBfile(const string &pdbFile, const PDB::Selector &selector = PDB::Selector());
    ...
    int
    readHetAtomsFromPDBfile(istream &PDBstream, const PDB::Selector &selector = PDB::Selector());
    ...
    // desired. <br> Note that to filter out hydrogen, water molecules
    // or anything else, use the PDB::WaterHydrogenUnSelector
    int readAllPDBfile(istream &PDBstream, const PDB::Selector &selector = PDB::Selector());
    ...
    {
    public:
        virtual bool operator()(const ParticleT *) const
        {
            return true;
        }
    ...
    //  friend ostream& operator<< <> (ostream& s, const Molecule<ParticleT>& m);
// #if 0 //The latter does not compile under gcc 4.0
// #ifdef g29
//   friend ostream& operator<<(ostream& s, const Molecule& m);
// #else
//   friend ostream& operator<<<>(ostream& s, const Molecule& m);
...
// #endif
private:
    typedef vector<ParticleT> BaseC;
...
template<class ParticleT>
int Molecule<ParticleT>::readPDBfile(istream &PDBstream,
                                     const PDB::Selector &selector)
{
    ...
    return vector<ParticleT>::size();
}


template<class ParticleT>
int Molecule<ParticleT>::readPDBfile(const string &pdbFile,
                                     const PDB::Selector &selector)
{
    ifstream pdbStream(pdbFile.c_str());
...
template<class ParticleT>
int Molecule<ParticleT>::readHetAtomsFromPDBfile(istream &PDBstream,
                                                 const PDB::Selector &selector)
{
    while (!PDBstream.eof())
    {
        std::string line;
        getline(PDBstream, line);
        //    string line=StringUtils::getLine(PDBstream);
...
template<class ParticleT>
int Molecule<ParticleT>::readAllPDBfile(istream &PDBstream,
                                        const PDB::Selector &selector)
{
    std::string line;
    while (!PDBstream.eof())
...
template<class ParticleT>
int Molecule<ParticleT>::readAllPDBfile(const string &pdbFile,
                                        const PDB::Selector &selector)
{
    ifstream pdbStream(pdbFile.c_str());
...
int Molecule<ParticleT>::select(const Selector &selector)
{
    ...
    for (base_const_iterator it = vector<ParticleT>::begin(); it != vector<ParticleT>::end(); ++it)
    {
        if (selector(&(*it)))
        {
            m.add((*it));
...
template<class ParticleT>
vector<Vector3> Molecule<ParticleT>::getDiameterTriple() const
{
    vector<Vector3> triple(3);
...
{
    for (base_iterator i = this->begin(); i != this->end(); ++i)
    { (*i) *= lt; }
}
...
{
    return (Molecule<ParticleT> &) vector<ParticleT>::operator=(m);
}

template<class ParticleT>
ostream &operator<<(ostream &s, const Molecule<ParticleT> &m)
{
    for (typename vector<ParticleT>::const_iterator it = m.begin();
         it != m.end(); ++it)
    {
        s << *it << endl;
    }
