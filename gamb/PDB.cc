...
static const string PROTEIN_RESIDUES =
"ALAA ARGR ASPD ASNN CYSC GLNQ GLUE GLYG HISH ILEI LEUL LYSK METM PHEF PROP SERS THRT TYRY TRPW VALV AS B GL Z UNKX";

static const unsigned short NUM_OF_NUCLEIC_ACIDS = 5;
static const string NUCLEIC_ACIDS = "ADEA CYTC GUAG THYT URAU UNKX";
...
bool PDB::isNucleicRecord(const std::string& PDBline) {
  const string residueName = getAtomResidueLongName(PDBline);
...
bool PDB::isNucleicAcid(const std::string& residueName)
{
  if (residueName[0] == 'U' && residueName[1] == 'N' && residueName[2] == 'K') {
    cerr << "PDB::isNucleicRecord - The given residue name is unknown :" <<residueName<<":"<< endl;
    return false;
  }
...
char PDB::residueShortName(const char* const resName)
{
  ...
  for (unsigned int i = 0 ; i < NUM_OF_NUCLEIC_RESIDUES ; i++) {
    if (resName[0] == NUCLEIC_RESIDUES[i][0] && resName[1] == NUCLEIC_RESIDUES[i][1] &&
...
string const PDB::residueLongName(const char resChar)
{
  string retName;
  ...
  cerr << "PDB::residueLongName - A unique long name does not exist for non-protein residue :" <<resChar<<":"<< endl;
  return retName;
}

string const PDB::nucleicAcidLongName(const char resChar)
{
  string retName;
  ...
  cerr << "PDB::nucleicAcidLongName - A unique long name does not exist for nucleic acid:" <<resChar<<":"<< endl;
  return retName;
}
...
AtomEntryType PDB::getAtomEntryType(const std::string& PDBline)
{
  ...
  string entryType = PDBline.substr(atomEntryType, field_len);
...
vector<unsigned short> PDB::connectedAtoms(const std::string& PDBline)
{
  vector<unsigned short> connAtoms;
