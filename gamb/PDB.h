...
<LI> 23/03/08 Dina: nucleic acid long names added, isNucleicAcid added
<LI> 23/03/08 Dina: WaterUnSelector added

<LI> 13/11/07 Elad Donsky: adding support for alternative location indicator
                           adding selector IgnoreAlternativesSelector

<LI> 10/05/05 Nelly Bluvshtein: Adding "isCONECTrec" method
                                Adding "atomResidueICode" method
                                Removing "atomResidueSequenceID" method (needed only by Atom)
</LI>
<LI> 21/9/04 Oranit Dror: (i) updating the 'NucleicAcidSelector' and
the 'CBetaSelector; inner classes; (ii) updating the
'residueLongName', 'atomResidueType' and 'getAtomEntryType' methods;
and (iii) adding the 'isNucleicRecord' method.
<LI> 25/8/04 Oranit Dror: Adding the 'NucleicAcidSelector' inner class
<LI> 10/6/04 Oranit Dror: Supporting Nucleic Acid PDB files
<LI> 17/3/04 Selectors added.  </UL>
...
public:
  ...
  static string const residueLongName(const char resChar);
  ...
  static string const getAtomResidueLongName(const std::string& pdb_line);
  ...
  ////
  //
  static vector<unsigned short> connectedAtoms(const std::string& pdb_line);
  ...
  class CBetaSelector: public Selector{
   public:
    bool operator()(const char *PDBrec) const {
      const string residueName = getAtomResidueLongName(PDBrec);
